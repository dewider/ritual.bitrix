document.addEventListener("DOMContentLoaded", function(e){
    ...
    jQuery(document).on('click', '.size_wrapper .size .size_item', function(e) {
        ...
        let id = $this.data('id');
        let $offer = $('#offers-hidden .item[data-id="'+id+'"]');
        let price = $offer.find('.price').html();
        let quantity = $offer.find('.quantity').html();

        $('.content_item .product-price').html(price);
        $qty = $('.qty input[type="number"]');
        $qty.prop('max', quantity);
        $qty.val( Math.min( $qty.val(), quantity ));
    });
